...
export default {
  ...
  async created() {
    ...
    const tickersData = window.localStorage.getItem('cryptonomicon-list') 
    if (tickersData) {
  ...
  methods: {
    subscribeToUpdates(tickerName) {
      setInterval(async () => {
        const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${tickerName}&tsyms=USD&api_key=eeb255d1edccb58e6ffd64c87d5b26e54c6346ef4b6e1cc8faa4c3d053834a0f`)
    ...
    add() {
      ...
      window.localStorage.setItem('cryptonomicon-list', JSON.stringify(this.tickers))
      this.ticker = ''
